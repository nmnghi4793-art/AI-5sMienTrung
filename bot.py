...
# =================== FORCE SIMPLE BULLETS (FINAL OVERRIDE) ===================
SIMPLE_BANKS_VERSION = "simple-bullets-2025-08-18"
...
def __simple_compose(kv_key: str|None, parts: dict|None=None) -> str:
    issues, recs = __simple_diag(kv_key or "HangHoa", parts or {})
    out = []
    if issues:
...
def _apply_simple_bullets_hooks():
    # Redirect all possible entry points to the simple version
    g = globals()
    g["_diagnose_varied"] = __simple_diag
    g["_compose_diagnostics"] = lambda kv, parts=None: __simple_compose(kv, parts or {})
    g["compose_feedback"] = lambda kv, parts=None: __simple_compose(kv, parts or {})
    g["compose_simple_feedback"] = g["compose_feedback"]
    g["compose_feedback_forced"] = g["compose_feedback"]
    print("[BOOT]", SIMPLE_BANKS_VERSION, "- hooks installed")
# =================== END FORCE SIMPLE BULLETS ===================
import pandas as pd
...
SCORING_ENABLED = os.getenv("SCORING_ENABLED","1") == "1"
...
def build_app() -> Application:
    ...
    app.add_handler(CommandHandler("version", lambda u,c: u.message.reply_text(f"✅ Using {SIMPLE_BANKS_VERSION}")))
