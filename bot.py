...
# ========== SIMPLE BULLET BANKS (override old diagnose) ==========
...
def _diagnose_varied(kv_key: str, parts: dict) -> tuple[list, list]:
    """Override – luôn dùng bộ câu mới, bỏ hoàn toàn bank cũ."""
    import random
    random.seed(hash(f"{kv_key}-{len(parts)}") % (2**32))
    def _pick(pool, k=5):
        pool = list(dict.fromkeys([x for x in pool if x]))  # khử trùng lặp, giữ thứ tự
        k = min(k, len(pool))
        return random.sample(pool, k) if k > 0 else []
    issues = _pick(SIMPLE_BULLET_ISSUES, 5)
    recs   = _pick(SIMPLE_BULLET_RECS,   5)
    return issues, recs
