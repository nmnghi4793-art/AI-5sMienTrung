...
def _extract_id_date_loose(text: str):
    """
    Bắt id_kho (>=6 chữ số) và ngày dd/mm/yyyy ở bất kỳ đâu trong caption.
    Nếu không có ngày → dùng hôm nay theo TZ.
    """
    if not text:
        text = ""
    import re as _re
    ...
    if m_day:
        ...
        now = datetime.now(TZ)
        d = datetime(now.year, now.month, now.day, tzinfo=TZ)
    return id_kho, d
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # dùng caption_any (caption của ảnh hiện tại nếu có)
    caption_any = caption_from_group or caption
    # Fallback: nếu parse cũ không ra id_kho/ngày thì bắt rộng từ caption_any
    if not id_kho or not isinstance(d, datetime):
        try:
            _id2, _d2 = _extract_id_date_loose(caption_any)
            id_kho = id_kho or _id2
            d = d if isinstance(d, datetime) else _d2
        except Exception:
            pass
