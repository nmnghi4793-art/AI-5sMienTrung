...
def __simple_pick(pool, k=5):
    import random
    pool = [x for x in pool if x]
    pool = list(dict.fromkeys(pool))  # khử trùng lặp
    k = min(k, len(pool))
    random.seed(hash(tuple(pool)) % (2**32))
...
def __simple_compose(kv_key: str|None, parts: dict|None=None) -> str:
    ...
    secs = []
    if issues:
        secs.append("⚠️ Vấn đề:\n" + "\n".join(f" • {s}" for s in issues))
    if recs:
        secs.append("\n🛠 Khuyến nghị:\n" + "\n".join(f" • {s}" for s in recs))
    return "\n".join(secs).strip()

def __force_hooks():
    # ép tất cả đường gọi về hàm mới
    for name in ("_diagnose_varied", "_compose_diagnostics", "compose_feedback",
                 "compose_simple_feedback", "compose_feedback_forced"):
        globals()[name] = (__simple_diag if "diagnose" in name else __simple_compose)
    # nếu phần gộp tin có biến dup_txt -> bỏ hiển thị “Trùng ảnh”
    try:
        globals()["dup_txt"] = ""
    except Exception:
        pass
